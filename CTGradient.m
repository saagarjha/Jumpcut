...
- (CTGradient *)removeColorStopAtPosition:(float)position {
    ...
    if (isnan(removedElement.position)) [NSException raise:NSRangeException format:@"-[%@ removeColorStopAtPosition:]: no such colorStop at position (%f)", [self class], position];
...
- (CTGradient *)removeColorStopAtIndex:(unsigned)index {
    ...
    if (isnan(removedElement.position)) [NSException raise:NSRangeException format:@"-[%@ removeColorStopAtIndex:]: index (%i) beyond bounds", [self class], index];
...
- (void)radialFillRect:(NSRect)rect {
    ...
    if (NSHeight(rect) > NSWidth(rect)) endRadius = NSHeight(rect) / 2;
    else endRadius = NSWidth(rect) / 2;
...
- (void)setBlendingMode:(CTGradientBlendingMode)mode;
{
    ...
    if (gradientFunction != NULL) CGFunctionRelease(gradientFunction);
...
- (CTGradientElement *)elementAtIndex:(unsigned)index;
{
    ...
    while (currentElement != nil) {
        if (count == index) return currentElement;
...
void resolveHSV(float *color1, float *color2) { //H value may be undefined (i.e. graycale color)
    ...
    if (isnan(color1[0]) && isnan(color2[0])) color1[0] = color2[0] = 0;
    else if (isnan(color1[0])) color1[0] = color2[0];
    else if (isnan(color2[0])) color2[0] = color1[0];
}
